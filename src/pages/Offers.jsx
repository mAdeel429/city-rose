...
//     } else {
//       const itemWithCategory = { ...item, category: 'Offers' };
//       addToFavorites(itemWithCategory);
//       setShowBubbles(true);
...
export default function Offers() {
  ...
  // Get device_id once component mounts
  useEffect(() => {
    const storedDeviceId = localStorage.getItem('device_id');
    setDeviceId(storedDeviceId);
  }, []);

  // Fetch offers when selectedType or deviceId changes
  useEffect(() => {
    const getOffers = async () => {
    ...
    if (isLoading || !deviceId) {
      return (
        <div style={{ textAlign: 'center', padding: '20px' }}>
          ...
          <p style={{ color: 'white' }}>Loading offers...</p>
        </div>
      );
    }
