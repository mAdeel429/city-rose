...
useEffect(() => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const pointData = await fetchPointDetailsWithPhotos(pointId);
      if (pointData?.photos?.length) {
        setImages(pointData.photos);
      }
    };
  ...
  return (
    <div>
      ...
      {images.map((img) => (
        <img
          key={img.id}
          src={`${img.url}?token=${localStorage.getItem('token')}`}
          alt={img.name}
